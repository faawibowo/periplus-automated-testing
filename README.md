...
|---|---|---|
| 1 | Test Case ID | A unique identifier for the test case (e.g., TC-001). |
...
| 4 | Preconditions | The required system state before execution (e.g., user logged in). |
...
| :--- | :--- |
| **Test Case ID** | TC-001 |
| **Test Case Name** | Add single product to cart and verify its name and price |
...
| **Preconditions** | User is successfully logged in. Cart is completely empty (cleared via @BeforeMethod). |
| **Test Data** | Search Keyword: `search.keyword` |
| **Test Steps** | 1. Search for a product using the designated keyword.<br>2. Click on the first product from the search results.<br>3. Retrieve and store the expected product name and product price.<br>4. Click the "Add to Cart" button.<br>5. Navigate to the Cart page. |
| **Expected Result** | The exact product should be present in the cart, and the cart's total price should match the product's unit price for a single item. |
...
| :--- | :--- |
| **Test Case ID** | TC-002 |
| **Test Case Name** | Adding same product twice results in total quantity of 2 |
...
| **Test Data** | Search Keyword: `search.keyword` |
| **Test Steps** | 1. Search for a product, select the first result, and click "Add to Cart".<br>2. Navigate back to the search results page.<br>3. Click the same product and click "Add to Cart" again.<br>4. Navigate to the Cart page.<br>5. Loop through the cart items to calculate the total quantity. |
...
| :--- | :--- |
| **Test Case ID** | TC-003 |
| **Test Case Name** | Adding different products all appear in cart |
...
| **Preconditions** | User is successfully logged in. Cart is completely empty. |
| **Test Data** | Keyword 1: `search.keyword`<br>Keyword 2: `search.keyword2` |
| **Test Steps** | 1. Search for the first keyword, select the first product, and add it to the cart.<br>2. Navigate back to the home page.<br>3. Search for the second keyword, select the first product, and add it to the cart.<br>4. Navigate to the Cart page.<br>5. Count the number of distinct items in the cart. |
...
| :--- | :--- |
| **Test Case ID** | TC-004 |
| **Test Case Name** | Cart badge count updates correctly after adding product |
| **Objective** | Verify that the cart badge counter on the navigation bar correctly increments after a product is added. |
| **Preconditions** | User is successfully logged in. Cart is completely empty. |
| **Test Data** | Search Keyword: `search.keyword` |
| **Test Steps** | 1. Search for a product and add it to the cart.<br>2. Retrieve the cart badge count text from the navigation bar on the home page. |
...
| :--- | :--- |
| **Test Case ID** | TC-005 |
| **Test Case Name** | Remove one product from cart |
| **Objective** | Verify that a user can successfully remove a product from the shopping cart, decreasing the item count. |
| **Preconditions** | User is successfully logged in. Cart contains at least 1 product. |
| **Test Data** | Search Keyword: `search.keyword` |
| **Test Steps** | 1. Navigate to the Cart page.<br>2. Record the initial item count in the cart.<br>3. Click the "Remove" button on the first product.<br>4. Verify the new item count and check if the product name still exists in the cart. |
...
| :--- | :--- |
| **Test Case ID** | TC-006 |
| **Test Case Name** | Increasing quantity updates total price correctly |
...
| **Test Steps** | 1. Navigate to the Cart page.<br>2. Retrieve and record the initial total price.<br>3. Click the "Increase Quantity" button for the product at index 0.<br>4. Retrieve the updated total price and compare it with the initial price. |
...
| :--- | :--- |
| **Test Case ID** | TC-007 |
| **Test Case Name** | Total price equals unit price times quantity |
...
| **Test Steps** | 1. Navigate to the Cart page and record the total price at quantity 1.<br>2. Set the product quantity manually to 3 using JavaScript execution.<br>3. Record the new total price at quantity 3.<br>4. Parse the prices and verify if the new total equals the initial price multiplied by 3. |
...
| :--- | :--- |
| **Test Case ID** | TC-008 |
| **Test Case Name** | Total price is correct sum of all different items |
| **Objective** | Verify that the overall cart total price is the correct cumulative sum of multiple distinct items' unit prices. |
| **Preconditions** | User is successfully logged in. Cart is completely empty. |
| **Test Data** | Product 1 Keyword: `search.keyword`<br>Product 2 Keyword: `search.keyword2` |
| **Test Steps** | 1. Search for the first product, parse its price, and add it to the cart.<br>2. Navigate to the base URL.<br>3. Search for the second product, parse its price, and add it to the cart.<br>4. Navigate to the Cart page.<br>5. Parse the grand total price and compare it to the sum of the two parsed item prices. |
...
| :--- | :--- |
| **Test Case ID** | TC-009 |
| **Test Case Name** | Cart persists after page refresh |
| **Objective** | Verify that the cart contents persist and are not lost when the user refreshes the browser page. |
| **Preconditions** | User is successfully logged in. Cart contains exactly 1 product. |
| **Test Steps** | 1. Navigate to the Cart page.<br>2. Assert that the added product is present in the cart.<br>3. Refresh the browser page.<br>4. Re-initialize the CartPage object and verify the product's presence again. |
...
| :--- | :--- |
| **Test Case ID** | TC-010 |
| **Test Case Name** | Cart persists after logout and re-login |
| **Objective** | Verify that the cart contents are permanently tied to the user's account and persist across different login sessions. |
| **Preconditions** | User is successfully logged in. Cart contains exactly 1 product. |
| **Test Data** | Valid user credentials (`test.email`, `test.password`). |
| **Test Steps** | 1. Add a product to the cart.<br>2. Perform the logout action to end the session.<br>3. Log back in using the identical user credentials.<br>4. Navigate to the Cart page.<br>5. Verify if the previously added product is still present. |
...
|---|---|---|---|---|---|
| TC-001 | addSingleProductToCart | Positive | Product appears in cart, total matches unit price | Matches | **PASS** |
| TC-002 | addSameProductTwiceIncreasesQuantity | Positive | Total quantity should be 2 | Total quantity remained 1 | **FAIL** |
| TC-003 | addMultipleDifferentProductsToCart | Positive | Cart contains 2 different items | 2 distinct items present | **PASS** |
...
| TC-006 | increaseQuantityUpdatesTotalPrice | Positive | Total price changes after quantity increase | Updated correctly | **PASS** |
...
## Video Demonstration
You can watch the automated test execution in action here: [Periplus Automated Testing Demo](https://youtu.be/M9PJtBAYgj8)
